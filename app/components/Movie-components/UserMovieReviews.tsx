      ...
      {reviewsForMovie && reviewsForMovie.length > 0 ? (
        <div className="border border-gray-300 shadow-gray-600 text-slate-900 bg-[#5d676f] p-4 rounded-lg shadow-md w-80 h-64 md:w-60 md:h-96 lg:h-96 lg:w-72 overflow-auto user-review-container">
          {reviewsForMovie.map((review: any) => {
            return (
              <div key={review.reviewId} className="my-2 p-4 rounded-lg shadow-md border border-gray-400 bg-slate-300 user-review-text-container">
                <div className="flex justify-between gap-2 md:gap-6 mb-1">
                <span className="text-xs font-semibold user-review-date">{new Date(review.date).toLocaleString()}</span> {/*The date is converted based on the user's location */}
                  <span className="text-xs font-semibold max-w-[4rem] overflow-hidden text-ellipsis whitespace-nowrap">{review.username}</span>
                </div>
