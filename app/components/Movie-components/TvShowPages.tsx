...
const TvShowPages = () => {
  ...
  return (
    <div className="flex flex-row mb-2 mt-1 lg:ml-8 text-center justify-center gap-4 text-lg not-search main-shows-pages not-search">
      <Link
        className="hover:opacity-[0.5] transition duration-200"
        href={`/Movies/TVShows/TVShowsPage/${page - 1}`}
      >
        {"<"}
      </Link>
      {pages.map((pageNumber) => (
        <div key={pageNumber}>
          <Link
            href={`/Movies/TVShows/TVShowsPage/${pageNumber}`}
            className={`${
              pageNumber === page
