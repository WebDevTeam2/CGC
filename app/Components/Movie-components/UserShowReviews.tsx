...
const UserShowReviews = async ({ showId }: { showId: Number }) => {
  ...
  const reviewsForShows = allUsers
    .flatMap(
      (user: any) =>
        (user.user_show_reviews || []).map((review: any) => ({
          ...review,
          username: user.username || user.name,
        })) //We want to be able to also display the username of the user who made the review
    )
    .filter((review: any) => review.showId === showId)
    .sort(
      (a: any, b: any) =>
  ...
  return (
    <>
      {reviewsForShows && reviewsForShows.length > 0 ? (
        <div className="border border-gray-300 shadow-gray-600 bg-[#5d676f] p-4 rounded-lg shadow-md w-80 h-64 md:w-60 md:h-96 lg:h-96 lg:w-72 overflow-auto user-review-container">
          {reviewsForShows.map((review: any) => {
            return (
              <div
                key={review.reviewId}
...
export default UserShowReviews;
