...
const AddToWatchlist = ({ id, media_type }: AddToWatchlistProps) => {
  ...
  useEffect(() => {
      ...
      if (session?.user?.email) {
        try {
          ...
          if (data?._id) {
            ...
            // Check if the movie or show is already in the user's watchlist
    ...
    fetchProfileDetails();
  }, [session?.user?.email, id]); // Add id as dependency
