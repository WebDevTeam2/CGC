"use client";
import { imageURL } from "@/app/Constants/constants";
...
const ShowReviews = ({ params }: { params: { id: string } }) => {
  ...
  useEffect(() => {
    const fetchShowData = async () => {
      try {
        const response = await fetch(`/api/Movies/FetchShow/${showId}`);
        const data = await response.json();

        const showsWithMediaType = {
          ...data,
          media_type: "tv",
        };
        setShowData(showsWithMediaType);
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      const res = await fetch(`/api/review/shows`, {
        method: "POST",
        headers: {
      ...
      //redirecting user to the tv show page
      setTimeout(() => {
        alert(
          `Review added successfully, redirecting to ${showData?.name} page`
        );
        router.push(`/Movies/TVShows/${showId}`);
        router.refresh();
      }, 1000);
    } catch (err) {
    ...
    return (
        <div className="flex flex-col gap-2 justify-center text-center md:mt-12 lg:mt-16 items-center not-search">
          ...
          <div className="flex justify-center gap-4 items-center md:mb-2 review-page">
            <div className="relative w-56 h-72 md:w-72 md:h-96 lg:w-72 lg:h-96">
              <img
                src={`${imageURL}${showData?.poster_path}`}
