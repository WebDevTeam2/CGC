...
const fetchAndCombineData = async () => {
  ...
  const fetchPromises = years.map((year) => {
    ...
    return getGameData(yearUrl, 1);
  });

  const results = await Promise.all(fetchPromises);
  results.forEach((result) => allGames.push(...result.results));
...
const Posts = ({ params }: { params: { page: number } }) => {
  ...
  const handleSort = (sortedGames: PostResult[]) => {
  ...
  if (loading) {
    return (
      <div className="w-full h-screen bg-black flex items-center justify-center">
        <Image
          src="https://media1.giphy.com/media/v1.Y2lkPTc5MGI3NjExYW50OWNpNWphNmRpem1pbWpwNjZiZGt2N3FneGc5ZXR0NzZidXpnOCZlcD12MV9pbnRlcm5hbF9naWZfYnlfaWQmY3Q9Zw/3o6vXTpomeZEyxufGU/giphy.webp"
          alt="loading"
          width={800}
          height={800}
        />
  ...
  const paginatedGames = paginateGames(games, params.page, pageSize);
  ...
  return (
    <div>
      <MainPage>
        ...
        <Sort games={games} onSorted={handleSort} />
        ...
        <Buttons gamesLength={games.length} />
      </MainPage>
    </div>
  );
};
